...
class EarlyStopping():
    def __init__(self, mode='higher', patience=50, filename=None, metric=None, n_fold=None, folder=None):
        ...
        if filename is None:
            filename = os.path.join(folder, 'early_stop.pth'.format(n_fold))
...
def load_dataloader(n_fold,args):
    ...
    celllines_data = osp.join(data_root,'Drugcomb/cell_feature_1061genes.npy')
    drugs_data = osp.join(data_root, 'Drugcomb/drugsmiles.npy')

    tr_data_items = osp.join(data_root, f'Drugcomb/{n_fold}_fold_train_DC.npy')   #0_fold_tr_cell0_items.npy
    val_data_items = osp.join(data_root, f'Drugcomb/{n_fold}_fold_val_DC.npy')
    test_data_items = osp.join(data_root, f'Drugcomb/{n_fold}_fold_test_DC.npy')
